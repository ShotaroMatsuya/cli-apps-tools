...
def create_table():
    c.execute(
        "CREATE TABLE IF NOT EXISTS jobstable(name TEXT,address TEXT,email TEXT,title TEXT,jobtype TEXT,salary INTEGER,status TEXT)"
    )


def add_data(name, address, email, title, jobtype, salary, status):
    c.execute(
        "INSERT INTO jobstable(name,address,email,title,jobtype,salary,status) VALUES (?,?,?,?,?,?,?)",
        (name, address, email, title, jobtype, salary, status),
    )
    conn.commit()
...
def edit_job_title(title, new_title):
    c.execute(
        'UPDATE jobstable SET title ="{}" WHERE title="{}"'.format(new_title, title)
    )
    conn.commit()
    data = c.fetchall()
    return data
...
def edit_job_status(status, new_status):
    c.execute(
        'UPDATE jobstable SET status ="{}" WHERE status="{}"'.format(new_status, status)
    )
    conn.commit()
    data = c.fetchall()
    return data
...
@click.option(
    ...
    "-jt",
    help="Job Type",
    type=click.Choice(["Full-Time", "Part-Time", "Remote", "Contract"]),
    prompt=True,
)
...
@click.option(
    ...
    "-st",
    help="Job Status",
    type=click.Choice(["Pending", "Cancelled", "Success"]),
    prompt=True,
)
...
def add_job(name, address, email, title, jobtype, salary, status):
    ...
    eg. jobtracker add-job --name JCharisTech --address Kumasi --email jcharis@gmail.com --title DataScientist --jobtype Remote --salary 140000 --status Pending
...
@main.command(
    cls=HelpColorsCommand, help_headers_color="yellow", help_options_color="magenta"
)
@click.option("--title", "-t", prompt=True)
def view_job(title):
    """View Job By Title

    eg.	jobtracker view-job --title "Developer"
    eg.  jobtracker view-job -t "Data Scientist"
    """
...
@click.argument("text")
@click.option(
    "--by", "-b", default="title", type=click.Choice(["name", "title", "status"])
)
def search(text, by):
    """Search Job By Options [Name or Title or Status]
    eg  jobtracker search "Developer" --by="title"
    """
    ...
    if by == "title":
        ...
        click.secho(
            "{} Not a Choice ,Pls Try either of these [name/title/status]".format(by),
            bg="red",
        )
...
def edit_job(field, old, new):
    ...
    eg. jobtracker edit-note --field="author" --old="Jesse" --new="JCharis"
    """
    click.secho(
        "Editing Field:: {} with {} and Updating to {}".format(field, old, new),
        fg="yellow",
    )
    from terminaltables import AsciiTable

    click.echo("===========Previous==============")
    result2 = view_all_jobs()
    new_result = [
        "Company Name",
        "Address",
        "Email",
...
def delete_job(title):
    """Delete Job By Title
    eg. jobtracker delete-job --title "Developer"
    """
